...
-- Table `MOU`.`customer`
-- -----------------------------------------------------
CREATE TABLE IF NOT EXISTS `MOU`.`customer` (
  ...
  CONSTRAINT `fk_customer_segment1`
    ...
    REFERENCES `MOU`.`segment` (`segment_id`)
    ON DELETE NO ACTION
    ON UPDATE NO ACTION)
ENGINE = InnoDB;
...
CREATE TABLE IF NOT EXISTS `MOU`.`mou_agreement` (
  `mou_agreement_id` INT NOT NULL AUTO_INCREMENT,
